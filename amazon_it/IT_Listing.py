...
def get_1(q_test, q_root,mutex,filename):
    while True:
        if q_test.empty():
            ...
            time.sleep(0.5)
            res = response(u[0])
            goal = res.xpath('//span[contains(text(),"Non disponibile")]')
            if goal:
                mutex.acquire()
                fp = open(filename,'a',encoding='utf8')
                lis = res.xpath('//ul[@id="s-results-list-atf"]/li[contains(@id,"result")]')
                for i in lis:
                    aim = i.xpath('.//span[contains(text(),"Non disponibile")]')
                    if aim:
                        ...
                        if asin1:
                            ...
                            n += 1
                            listing_url1 = 'https://www.amazon.it/dp/' + asin1[0]
                            asin.append(asin1[0])
                        ...
                        if title1:
                            ...
                            title.append('')
                        rate1 = i.xpath(".//span[@class='a-icon-alt']/text()")
...
def main():
    ...
    filename = '%s年%s月%s日_无主ASIN采集数据.csv' % (a.year, a.month, a.day)
    # fp = open(filename, 'a', encoding='utf8')
    ...
    ft = open(filename_2, 'a', encoding='utf8')
    ft.write('url,title\n')
    ft.close()
    # y = 1
    try:
