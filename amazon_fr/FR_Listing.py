...
def get_1(q_test, q_root,mutex,filename):
    while True:
        if q_test.empty():
            ...
            time.sleep(0.5)
            res = response(u[0])
            goal = res.xpath('//span[contains(text(),"Actuellement indisponible")]')
            if goal:
                mutex.acquire()
                fp = open(filename,'a',encoding='utf8')
                lis = res.xpath('//ul[@id="s-results-list-atf"]/li[contains(@id,"result")]')
                for i in lis:
                    aim = i.xpath('.//span[contains(text(),"Actuellement indisponible")]')
                    if aim:
                        # fp = open(filename,'a',encoding='utf8')
                        ...
                        if asin1:
                            ...
                            n += 1
                            listing_url1 = 'https://www.amazon.fr/dp/' + asin1[0]
                            asin.append(asin1[0])
...
def main():
    ...
    filename = '%s年%s月%s日_法国无主ASIN采集数据.csv' % (a.year, a.month, a.day)
    # fp = open(filename, 'a', encoding='utf8')
