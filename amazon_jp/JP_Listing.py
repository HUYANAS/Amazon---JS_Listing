...
def get_1(q_test, q_root,mutex,filename):
    while True:
        if q_test.empty():
            ...
            time.sleep(0.5)
            res = response(u[0])
            goal = res.xpath('//span[contains(text(),"現在お取り扱いできません") or contains(text(),"現在在庫切れです。")]')
            if goal:
                mutex.acquire()
                fp = open(filename,'a',encoding='utf8')
                lis = res.xpath('//ul[@id="s-results-list-atf"]/li[contains(@id,"result")]')
                for i in lis:
                    aim = i.xpath('.//span[contains(text(),"現在お取り扱いできません") or contains(text(),"現在在庫切れです。")]')
                    if aim:
                        # fp = open(filename,'a',encoding='utf8')
                        ...
                        if asin1:
                            ...
                            n += 1
                            listing_url1 = 'https://www.amazon.co.jp/dp/' + asin1[0]
                            asin.append(asin1[0])
...
def main():
    ...
    filename = '%s年%s月%s日_日本无主ASIN采集数据.csv' % (a.year, a.month, a.day)
    # fp = open(filename, 'a', encoding='utf8')
